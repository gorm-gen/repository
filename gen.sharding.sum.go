...
// genCount count.go
func (r *Repository) genShardingSum(rt reflect.Type, _data *base) error {
	data := struct {
		Package               string
		GenQueryPkg           string
		RepoPkg               string
	...
	file, err := os.Create(path.Join(_data.paths, "sharding.sum.gen.go"))
	if err != nil {
		return err
	}
	defer func() {
	...
	t, err := template.New(r.genShardingSumTemplate()).Parse(r.genShardingSumTemplate())
	if err != nil {
		return err
	}
