...
// genCount count.go
func (r *Repository) genShardingCount(rt reflect.Type, _data *base) error {
	data := struct {
		Package               string
		GenQueryPkg           string
		RepoPkg               string
	...
	file, err := os.Create(path.Join(_data.paths, "sharding.count.gen.go"))
	if err != nil {
		return err
	}
	defer func() {
	...
	t, err := template.New(r.genShardingCountTemplate()).Parse(r.genShardingCountTemplate())
	if err != nil {
		return err
	}
