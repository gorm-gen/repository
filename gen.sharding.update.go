...
// genCount count.go
func (r *Repository) genShardingUpdate(rt reflect.Type, _data *base) error {
	data := struct {
		Package               string
		GenQueryPkg           string
		RepoPkg               string
	...
	file, err := os.Create(path.Join(_data.paths, "sharding.update.gen.go"))
	if err != nil {
		return err
	}
	defer func() {
	...
	t, err := template.New(r.genShardingUpdateTemplate()).Parse(r.genShardingUpdateTemplate())
	if err != nil {
		return err
	}
