...
// genCount count.go
func (r *Repository) genShardingFirst(rt reflect.Type, _data *base) error {
	data := struct {
		Package               string
		GenQueryPkg           string
		RepoPkg               string
	...
	file, err := os.Create(path.Join(_data.paths, "sharding.first.gen.go"))
	if err != nil {
		return err
	}
	defer func() {
	...
	t, err := template.New(r.genShardingFirstTemplate()).Parse(r.genShardingFirstTemplate())
	if err != nil {
		return err
	}
