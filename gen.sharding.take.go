...
// genCount count.go
func (r *Repository) genShardingTake(rt reflect.Type, _data *base) error {
	data := struct {
		Package               string
		GenQueryPkg           string
		RepoPkg               string
	...
	file, err := os.Create(path.Join(_data.paths, "sharding.take.gen.go"))
	if err != nil {
		return err
	}
	defer func() {
	...
	t, err := template.New(r.genShardingTakeTemplate()).Parse(r.genShardingTakeTemplate())
	if err != nil {
		return err
	}
